...
// Quick-reference popover for the Lucky 9 rules. Same portal + clamped fixed
// positioning as the Pusoy Trese HandTypesMenu, so it floats above the table
// and the draw sheet instead of being trapped in their stacking context.
export default function RulesMenu({ themeClass }: { themeClass?: string }) {
	...
	const rules: [string, string][] = [
		...
		["Hand total", "Only the last digit of the sum counts (7 + 8 = 15 → 5)."],
		["Hirit", "After betting you may draw one extra card — three cards max."],
		["Lucky 9", "A two-card 9 is the best hand and pays double the bet."],
		["Ties", "Equal totals: fewer cards wins; same card count is a push."],
		["Banker", "Everyone plays the banker's hand. The bank rotates every 3 games."],
	];
